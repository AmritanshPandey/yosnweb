...
function Marker({
  ...
  const markerVisualSize = Math.max((marker.size ?? defaultSize) * 140, 8);
      ...
      <group ref={imageGroupRef} position={topPosition}>
        ...
        >
          <div
            ...
            style={{
              width: `${markerVisualSize}px`,
              height: `${markerVisualSize}px`,
            }}
...
function RotatingGlobe({
  ...
  // Load Earth textures
  const [rawEarthTexture, rawBumpTexture] = useTexture([
    config.textureUrl,
    config.bumpMapUrl,
  ]);

  // Configure textures
  const { earthTexture, bumpTexture } = useMemo(() => {
    const configuredEarthTexture = rawEarthTexture.clone();
    configuredEarthTexture.colorSpace = THREE.SRGBColorSpace;
    configuredEarthTexture.anisotropy = 16;

    const configuredBumpTexture = rawBumpTexture.clone();
    configuredBumpTexture.anisotropy = 8;

    return {
      earthTexture: configuredEarthTexture,
      bumpTexture: configuredBumpTexture,
    };
  }, [rawEarthTexture, rawBumpTexture]);
